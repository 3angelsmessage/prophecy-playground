      ...
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <p className="text-sm opacity-70 max-w-sm mb-4">{t("footer.brandDesc")}</p>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }}>
            ...
            <blockquote className="text-sm opacity-70 italic border-l-2 border-primary pl-4">
              {t("footer.scriptureText")}
              <footer className="mt-2 text-primary font-semibold not-italic">{t("footer.scriptureRef")}</footer>
            </blockquote>
          </motion.div>
        </div>
        ...
        <div className="pt-8 border-t border-background/10 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-sm opacity-70">
            {t("footer.copyright")} <Heart size={14} className="inline text-prophecy-coral" /> {t("footer.forStudents")}
          </p>
          <p className="text-sm opacity-70">{t("footer.sdaBased")}</p>
        </div>
      </div>
    </footer>
  );
